...
cat << EOF  
Usage: ./software-delivery-infra -[h]
...
-i,        GCP project ID for multi-tenant infrastructure

-p,        GCP project name for Tenant Factory

-r,        GitHub repo name that will host Tenant Factory code
...
-T,        Cloud Build trigger type - can be "webhook" to "github" trigger
...
while getopts ":o:b:fi:p:r:u:t:O:R:T:g:h" options; do
  case "${options}" in                    
    o)                                    
      ORG_NAME=${OPTARG}                    
      ;;
    ...
    f)                                   
      ...
      ;;
    i)                                   
      INFRA_SETUP_PROJECT=${OPTARG}                    
      ;;
    p)                                   
      APP_SETUP_PROJECT=${OPTARG}                    
      ;;
    r)                                   
      APP_SETUP_REPO=${OPTARG}                    
      ;;
    u)                                   
      GITHUB_USER=${OPTARG}                    
      ;;
...
echo "${ORG_NAME} ${BILLING_ACCOUNT_ID}  ${INFRA_SETUP_PROJECT} ${APP_SETUP_PROJECT} ${APP_SETUP_REPO} ${GITHUB_USER} ${TOKEN} ${GITHUB_ORG} ${REGION} ${TRIGGER_TYPE}  ${IAM_GROUP}"
if [ -z ${ORG_NAME} ] || [ -z ${BILLING_ACCOUNT_ID} ] || [ -z ${INFRA_SETUP_PROJECT} ] || [ -z ${APP_SETUP_PROJECT} ] || [ -z ${APP_SETUP_REPO} ] || [ -z ${GITHUB_USER} ] || [ -z ${TOKEN} ] || [ -z ${GITHUB_ORG} ] || [ -z ${REGION} ] || [ -z ${TRIGGER_TYPE} ] || [ -z ${IAM_GROUP} ] ; then
showHelp
fi
