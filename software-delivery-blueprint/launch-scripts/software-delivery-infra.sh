...
-p,        GCP project name for multi-tenant infrastructure

-r,        GitHub repo name that will host IaC to build multi-tenant infrastructure
...
while getopts ":o:b:fp:r:u:t:O:R:T:g:h" options; do
  case "${options}" in                    
    o)                                    
      ORG_NAME=${OPTARG}                    
      ;;
...
echo "${ORG_NAME} ${BILLING_ACCOUNT_ID}  ${INFRA_SETUP_PROJECT} ${INFRA_SETUP_REPO} ${GITHUB_USER} ${TOKEN} ${GITHUB_ORG} ${REGION} ${TRIGGER_TYPE}  ${IAM_GROUP}"
if [ -z ${ORG_NAME} ] || [ -z ${BILLING_ACCOUNT_ID} ] || [ -z ${INFRA_SETUP_PROJECT} ] || [ -z ${INFRA_SETUP_REPO} ] || [ -z ${GITHUB_USER} ] || [ -z ${TOKEN} ] || [ -z ${GITHUB_ORG} ] || [ -z ${REGION} ] || [ -z ${TRIGGER_TYPE} ] || [ -z ${IAM_GROUP} ] ; then
showHelp
fi
